...
const EmployerChangesTable = ({ data }) => {
  ...
  const getRandomAlumni = (employer, count) => {
    const shuffled = [...mockNames].sort(() => 0.5 - Math.random());
    return shuffled.slice(0, count).map(name => ({
      name,
      email: `${name.toLowerCase().replace(' ', '.')}@${employer.toLowerCase()}.com`,
  ...
  const columns = [
    ...
    {
      title: 'Other Alumni Working in same Employer',
      key: 'otherAlumni',
      render: (record) => {
        const otherAlumni = getRandomAlumni(record.employer, Math.floor(Math.random() * 3) + 1);
...
export default EmployerChangesTable;